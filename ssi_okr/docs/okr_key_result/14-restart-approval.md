# Restart Approval Process — OKR Key Result
...
- **Config:** An active `policy.template` for `okr_key_result` grants
  `restart_approval_ok` for state `open` to the actor's group, under the condition
  above.
- **Config:** An active `approval.template` for `okr_key_result` exists to be matched.
- **Access:** User is in group `Validator` (`okr_key_result_validator_group`).

## Flow

1. Open the **Project > Key Results** menu.
2. Open the Key Result whose approval process needs to be (re)prepared.
3. Click the **Restart Approval Process** button in the header.
4. Click **OK** on the confirmation dialog.

## Post-Condition

- Any existing approval records for this Key Result are removed.
